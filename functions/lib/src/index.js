...
function requireAdmin(request) {
    ...
    return db.doc(`users/${request.auth.uid}`).get().then((snapshot) => {
...
exports.reviewTeacherApplication = (0, https_1.onCall)(async (request) => {
    ...
    await db.runTransaction(async (transaction) => {
        ...
        const notification = applicantNotification(decision === "approve" ? "Your teacher account has been approved." : `Your teacher application needs updates: ${reason}`, applicant?.name || application?.name || "Teacher application");
        transaction.update(applicationRef, { status, rejectionReason: decision === "reject" ? reason : firestore_1.FieldValue.delete(), reviewedAt: now, reviewedBy: adminId, updatedAt: now });
        transaction.update(applicantRef, { type: decision === "approve" ? "teacher" : "student", teacherApprovalStatus: status, teacherReviewReason: decision === "reject" ? reason : firestore_1.FieldValue.delete(), notifications: firestore_1.FieldValue.arrayUnion(notification) });
        if (decision === "approve")
            transaction.set(teacherRef, { ...applicant, type: "teacher", teacherApprovalStatus: "approved", approvedAt: now }, { merge: true });
        transaction.set(auditRef, { applicationId, applicantId: applicationId, action: status, reason: decision === "reject" ? reason : "", adminId, createdAt: now });
    });
...
exports.resubmitTeacherApplication = (0, https_1.onCall)(async (request) => {
    ...
    await db.runTransaction(async (transaction) => {
        ...
        if (!applicationSnapshot.exists || applicationSnapshot.data()?.status !== "rejected") {
            throw new https_1.HttpsError("failed-precondition", "Only rejected applications can be resubmitted.");
        }
        const now = firestore_1.FieldValue.serverTimestamp();
        transaction.update(applicationRef, { status: "pending", rejectionReason: firestore_1.FieldValue.delete(), updatedAt: now, resubmittedAt: now });
        transaction.update(applicantRef, { type: "student", teacherApprovalStatus: "pending", teacherReviewReason: firestore_1.FieldValue.delete() });
        transaction.set(auditRef, { applicationId, applicantId: applicationId, action: "resubmitted", reason: "", adminId: null, createdAt: now });
    });
...
exports.remindOverdueTeacherApplications = (0, scheduler_1.onSchedule)("every day 09:00", async () => {
    ...
    const applications = await db.collection("teacherApplications").where("status", "==", "pending").where("createdAt", "<=", cutoff).get();
