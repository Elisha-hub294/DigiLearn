...
export function BookPreviewScreen() {
  const { id } = useLocalSearchParams<{ id: string }>(); const [book, setBook] = useState<Book>(); const [allBooks, setAllBooks] = useState<Book[]>([]); const [favourite, setFavourite] = useState(false); const [bookmarked, setBookmarked] = useState(false); const insets = useSafeAreaInsets(); const { width } = useWindowDimensions(); const [gradient] = useState(() => gradients[Math.floor(Math.random() * gradients.length)]);
